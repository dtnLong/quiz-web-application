<template>
  <section v-if="!isLogin" class="relative grid w-full h-screen place-items-center">
    ...
    <div class="flex flex-col items-center w-11/12 md:w-10/12 lg:w-8/12 pb-14 text-gray-50 rounded-xl bg-form-bg pt-9">
      ...
      <form @submit.prevent="authenticate" class="w-3/4 md:w-4/6">
        <div class='mb-8'>
      ...
      <button class="flex items-center gap-4 py-3 text-2xl font-bold transition-all duration-200 ease-linear rounded-full hover:bg-none hover:bg-error-900 px-7 bg-gradient-to-r from-red-600 to-pink-600" :class="{loading: isLoading}" @click.prevent="authenticate"> <div v-if="isLoading" class="inline-block w-4 h-4 ease-linear border-2 border-t-2 border-gray-200 rounded-full loader"></div> Start the quiz!
      ...
      <p class="px-2 mt-3 text-xs text-center text-gray-700 ">Forgot your quiz code? Please contact <a class="italic underline" href="mailto:thelogisticom.rmitbc@gmail.com" target="_blank" rel="noopener noreferrer">thelogisticom.rmitbc@gmail.com</a> to retrieve another one</p>
    </div>
  </section>
