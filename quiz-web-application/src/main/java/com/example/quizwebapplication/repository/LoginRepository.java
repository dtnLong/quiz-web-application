...
public interface LoginRepository extends JpaRepository<Login, Long> {
    ...
    @Query(value = "SELECT L.startTime FROM Login L WHERE L.group.name = :groupName AND L.quizCode.code = :quizCode")
    Optional<Date> findStartTimeByNameAndQuizCode(String groupName, String quizCode);

    @Query(value = "SELECT L.submitted FROM Login L WHERE L.group.name = :groupName AND L.quizCode.code = :quizCode")
    ...
    @Query(value = "UPDATE Login L SET L.expired = :expired WHERE L IN (" +
            "SELECT L1 FROM Login L1 WHERE L1.group.name = :name AND L1.quizCode.code = :quizCode)")
